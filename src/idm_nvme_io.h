...
/**
 * This struct represents the pieces of the status word that are returned
 * from ioctl().  This bit field definitions of the status are defined
 * in DWORD3[31:17] of the NVMe spec's Common Completion Queue Entry (CQE).
 * This DWORD3[31:17] appears to be what ioctl() returns. (ie- in status[14:0]))
*/
//TODO: Is struct useful or unnecessarily complicated?
...
//TODO: Need to fix mixed use of nvme_admin_cmd and nvme_passthru_cmd.  Pick 1.
...
struct idm_nvme_request {
	...
	//IDM core structs
	struct nvme_idm_vendor_cmd  cmd_nvme;  //Struct NOT used by ioctl()
	...
	//Variables used by custom asynchronous NVMe IO code (ANI)
	uuid_t                      uuid_async_job;
	struct arg_ioctl            *arg_async_nvme;
	struct nvme_passthru_cmd    *cmd_nvme_passthru;//struct used by ioctl()
};
